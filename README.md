...
Estimator, Projects, and CRM into one consistent product — plus a home Dashboard.
...
- **Unified shell & nav** — a single persistent top bar (logo · Dashboard /
  Catalog / Estimator / Projects / CRM · company name) on every page, with the
...
| --- | --- | --- |
| Dashboard | `src/pages/Dashboard.tsx` | Pipeline value, active clients, prospects, awaiting-signature count, won revenue this month, recent activity. |
...
| CRM | `src/pages/CRM.tsx` | Wide multi-column client cards, Won Revenue rollup, New Estimate prefill. |
| Client profile | `src/pages/ClientProfile.tsx` | Contact, estimates, "New Estimate" pre-filled from the client. |
...
- Setting an estimate to **Won** rolls its value into CRM **Won Revenue**.
